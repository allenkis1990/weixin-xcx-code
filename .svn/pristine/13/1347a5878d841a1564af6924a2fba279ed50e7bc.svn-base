...
/**
 *    ////////             登录页面逻辑         //////////
 * 先用账号密码去调用接口adminSystemUserLogin拿到userId
 * 调用haveBindWx判断此微信是否绑定过
 * 如果绑定过拿到haveBindWx接口返回的userId进行登录
 * 如果没绑定过 调用微信绑定账号口，绑定完后用adminSystemUserLogin返回的userId进行登录
 */
      ...
      app.requestData(app.config.adminSystemUserLogin, getUserIdParams, "POST").then(data => {
          ...
          if (data.head.code !== app.constant.network_result_success) {
              ...
              if(userId){
                  ...
                  var haveBindWxParams={
                      unionId: app.config.casConfig.unionId,
                      openId: app.config.casConfig.openId
                  };
                  app.requestData(app.config.haveBingWX, haveBindWxParams, "POST").then(data => {
                      wx.hideLoading()
                      if (data.head.code != app.constant.network_result_success) {
                          return wx.showToast({
                              title: data.head.message,
