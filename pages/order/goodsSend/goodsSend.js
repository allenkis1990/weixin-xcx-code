// pages/order/goodsSend/goodsSend.js
let app = getApp()
Page({
  ...
  loadSelfPickAddressListData() {
    ...
    app.requestData(app.config.orderSelfStorageList, params, 'GET').then(function (response) {
      ...
      if (response.head.code == app.constant.network_result_success) {
        ...
        //如果选中的是自取，则设置选中的item
        if (context.data.preCertificateSendInfo.deliverType !== undefined && context.data.preCertificateSendInfo.deliverType == 1 && context.data.preCertificateSendInfo.selfStorageId.length) {
          for (let index = 0; index < context.data.selfPickerAddressList.length; index++) {
            let item = context.data.selfPickerAddressList[index]
            if (item.selfStorageId == context.data.preCertificateSendInfo.selfStorageId) {
              context.setData({
  ...
  setUserAddressInfo() {
    ...
    if (userReceiveAddress !== undefined && userReceiveAddress.receiverId !== undefined && userReceiveAddress.receiverId.length) {
      // 获取选择当前选中地区的index
  ...
  */
  saveAction() {
    ...
    if (this.data.selectedDeliverType == 2) {
      if (this.checkInformation()) {
        ...
        app.requestData(app.config.orderUpdateUserReceiverAddress, params, 'POST').then(function (response) {
          if (response.head.code == app.constant.network_result_success) {
            ...
            prevPage.data.certificateSendInfo.deliverType = context.data.selectedDeliverType
            prevPage.data.certificateSendInfo.receiverAddressId = response.data.receiverId
            prevPage.data.certificateSendInfo.addressDetails = response.data.addressDetails
            prevPage.data.invoiceInfo.deliverType = context.data.selectedDeliverType
            prevPage.data.invoiceInfo.receiverAddressId = response.data.receiverId
            prevPage.data.invoiceInfo.addressDetails = cresponse.data.addressDetails
            prevPage.setData({
              certificateSendInfo: prevPage.data.certificateSendInfo,
              showCertificateSendText: '邮寄（' + context.data.userAddressInfo.addressDetails + ')'
            })
            wx.navigateBack()
          } else {
            wx.showToast({
      ...
      prevPage.data.certificateSendInfo.deliverType = context.data.selectedDeliverType
      prevPage.data.certificateSendInfo.selfStorageId = context.data.selectedSelfPickerAddress.selfStorageId
      prevPage.data.certificateSendInfo.selfStorageName = context.data.selectedSelfPickerAddress.selfStorageName
      prevPage.data.invoiceInfo.deliverType = context.data.selectedDeliverType
      ...
      prevPage.setData({
        certificateSendInfo: prevPage.data.certificateSendInfo,
        showCertificateSendText: '自取（' + context.data.selectedSelfPickerAddress.selfStoragePickupAddress + ')'
      })
      wx.navigateBack({})
    }
  },
  ...
  checkInformation() {
    ...
    if (!this.data.userAddressInfo.provinceId.length || !this.data.userAddressInfo.cityId.length || !this.data.userAddressInfo.districtId.length) {
